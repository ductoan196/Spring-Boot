...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Optional<Role> rolePartnerOptional = roleRepository.findByName(Roles.PARTNER);
        if (rolePartnerOptional.isEmpty()) {
            Role userRole = Role.builder().name(Roles.PARTNER).build();
            roleRepository.save(userRole);
        }
